...
fn vs_main(
    ...
    let transformed_point = uniforms.camera_transform * vec4<f32>(in_vertex.v_position, 1.0);
    let a = transformed_point.x + transformed_point.y + transformed_point.z;
    let f = 0.333333333;
    let real_int = floor(transformed_point) + (fract(transformed_point) / 1.2);
    
                                  // floor((transformed_point.x + a) * f),
                                  // floor((transformed_point.y + a) * f),
                                  //     floor((transformed_point.z + a) * f),
    out.clip_position = transformed_point;
        // vec4<f32>(real_int,
        //                           transformed_point.w);
...
fn fs_main(
    ...
    return color;// + (vec4<f32>(0.5, 0.0, 0.0, 0.0));
}
