...
pub fn geo<T>(u: Vector<T>, v: Vector<T>) -> MultiVector<T>
where
    T: Copy
        + std::ops::Mul<Output = T>
        + std::ops::Sub<Output = T>
        ...
        + std::fmt::Debug,
{
    ...
    MultiVector { grade, terms }
}
impl<T: Copy + std::ops::Mul<Output = T> + std::iter::Sum + number_traits::Float + From<f32>>
...
impl<T: Copy + std::ops::Mul<Output = T> + std::iter::Sum> std::ops::Mul for Vector<T> {
    type Output = Vector<T>;

    fn mul(self, rhs: Vector<T>) -> Vector<T> {
        let dimensions: Vec<T> = self
            .dimensions
            .iter()
            .enumerate()
            .map(|(i, d)| rhs.dimensions.iter().map(|r| *r * *d).sum())
            .collect();
        Vector { dimensions }
    }
}
